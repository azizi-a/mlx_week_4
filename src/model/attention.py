...
class Attention(nn.Module):
  def __init__(self):
    ...
    self.value = nn.Linear(EMBEDDING_DIM, LATENT_DIM)
    self.v_up = nn.Linear(LATENT_DIM, EMBEDDING_DIM)
  ...
  def forward(self, x, y, mask=None):
    ...
    v = self.value(y).view(y.size(0), -1, self.head_count, self.head_dim).transpose(1, 2)
    ...
    dx = torch.matmul(a, v)
    dx = dx.transpose(1, 2).contiguous().view(x.size(0), -1, self.latent_dim)
    dx = self.v_up(dx)
    x = x + dx
    x = self.norm_3(x)
